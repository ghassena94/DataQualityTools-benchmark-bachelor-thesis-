...
def print_rows(outliers, model, hints, rules_descriptions, verbosity = 0, max_w = 40, header = "   ", dataset_name = None):
    # REIN addition: upstream dBoost only prints outliers to stdout, but REIN's
    # detectors.py::dboost() consumes them from "<input>-dboost_output.csv", one
    # "linum,field_id" pair per flagged cell. linum counts the CSV's physical rows
    # with the header at 0, which is why the caller drops linum == 0 and shifts the
    # rest by -1 to get the dataframe row. Duplicate pairs are possible when several
    # discrepancy groups hit the same field; the caller dedupes via a dict.
    flagged_cells = []
    ...
    for linum, (x, X, discrepancies) in outliers:
        ...
        flagged_cells.extend((linum, field_id) for field_id in highlight)
    ...
    if dataset_name is not None:
        with open(dataset_name + "-dboost_output.csv", "w") as output:
            for linum, field_id in flagged_cells:
                output.write("{},{}\n".format(linum, field_id))
